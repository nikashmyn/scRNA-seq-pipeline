...
source('/pellmanlab/stam_niko/etai_code/DFCI.scRNAseq.workflows/plots/class_prob_to_cn_level.R')

#if MLREG is not provided then lgbmpreds5 is required to be pre-loaded
plot_raw_data_and_prediction_boxplots <- function(myid = "170512_B4", chr = "chr5", th = 10, 
                                                  adt, nonzeros.zs, coding, centromeres, eps = 0,
                                                  controlSampleIDs, doPlot = T) {
  require(ggplot2)
  require(matrixStats)
  ...
  #IS THIS NEEDED NOW THAT Z_SCORE IS FIXED
  ...
  aggsA <- merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), aggs.A.zs[seqnames == chr][, c("bin", myid), with=F])
  aggsAc <- (merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), coding$cnts.A[seqnames == chr][, c("bin", myid), with=F]))
  ...
  tmp2A <- merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), tmp)
  ...
  aggsB <- merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), aggs.B.zs[seqnames == chr][, c("bin", myid), with=F])
  aggsBc <- merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), coding$cnts.B[seqnames == chr][, c("bin", myid), with=F])
  ...
  if(length(wi)>0)
    tmp$value[tmp$bin == wi] = NA
  tmp2B <- merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), tmp)
  ...
  mynonzeros <- merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), mynonzeros0[seqnames == chr][, c("bin", myid), with=F])
  ...
  tmp <- melt(t(tmp))[,2:3]
  colnames(tmp) <- c("bin", "value")
  tmp2 <- merge(unique(tt2[method == "tpm", c("bin", "pos", "proportion")]), tmp)
  ...
  if(!doPlot) {
    ...
    predbins <- tt2[, lapply(.SD, median, na.rm=T), .SDcols="value", by = "bin"]
  ...
  p2 <- ggplot(tt2b, aes(x = pos, y = median, ymin = se.min, ymax = se.max)) + #, fatten = 6+proportion/max(proportion)*15)) + 
    ...
    ggtitle(sprintf("Normalized Ratio of Expressed Genes")) + ylim(c(0, 3.2)) + 
  ...
  p4 <- ggplot(data = tmp2AB, mapping = aes(x = pos, y= value, fill=method)) + ylim(c(-0.3,4))+
    ...
    scale_x_discrete(breaks=tt2$pos, labels = as.character(round(as.numeric(as.character(tt2$pos))/1e6))) + 
    xlab(label = sprintf("Position (Mbp) %s", chr)) 
  
  #return(list(p1, p2, p3, p4)) 

  #return(grid.arrange(p1, p2, p3, p4, nrow = 4))
