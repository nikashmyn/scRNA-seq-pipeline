...
#plot(unlist(adt.bychr[c(12),..controlSampleIDs])*2, unlist(var_mat.bychr[c(12),..controlSampleIDs])*2, xlim = c(0,5), ylim = c(0,5))
#plot(unlist(rowMeans(adt.bychr[,..controlSampleIDs]))*2, unlist(rowMeans(adt.bychr[,..controlSampleIDs]))*2, xlim = c(0,5), ylim = c(0,5))

setkey(anno, WTA.plate)
families <- sort(table(anno[Pairs != "NA" & Pairs != "mother"]$Pairs), decreasing = T)

#Prepare Visual Objects
myfamily = names(families)[30] #43, 24, 142
message(myfamily)
