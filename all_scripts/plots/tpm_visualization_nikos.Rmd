...
------------------------------------------------------------------------------
Graph: Genomic coordinates graph
This is the raw gene expression for chr1 with genomic position based bins.

```{r}
rsemtpm_nikos_chr1 <- rsemtpm_nikos[rsemtpm_nikos$seqnames == 'chr1']

first_genome_coord <- min(rsemtpm_nikos_chr1$start)
final_genome_coord <- max(rsemtpm_nikos_chr1$end)

bin_size <- 20000000

genome_bins <- as.integer(ceiling((final_genome_coord - first_genome_coord) / bin_size))
...
rsemtpm_nikos_chr1_binsummed <- colnames(rsemtpm_nikos_chr1[,-c(1:6)])
for ( i in 2:length(bins) ) {
  tmp <- rsemtpm_nikos_chr1[rsemtpm_nikos_chr1$end <= bins[i]]
  tmp <- tmp[bins[i-1] < tmp$end]
  col_sums <- colSums(tmp[,-c(1:6)])
  rsemtpm_nikos_chr1_binsummed <- rbind(rsemtpm_nikos_chr1_binsummed, col_sums)
}
rsemtpm_nikos_chr1_binsummed <- rsemtpm_nikos_chr1_binsummed[-c(1),]
...
df <- data.frame(sapply(rsemtpm_nikos_chr1_binsummed, function(x) {
  if(is.numeric(x) == F) {
...
boxplot(df2, use.cols = TRUE, outline=FALSE,
        main="Raw Gene Expression for Chr1", 
        xlab="Gene Bins (20 MB)", ylab="Expression (TPM)")
...
meds <- rowMedians(rsemtpm_nikos_control)

for ( i in 1:nrow(rsemtpm_nikos_control )) {
  if (meds[i] == 0) {
    meds[i] <- NaN
  }
  for ( j in 1:ncol(rsemtpm_nikos_control) ) {
    rsemtpm_nikos_control[i,j] <- rsemtpm_nikos_control[i,j]/meds[i]
  }
}

number_of_genes <- nrow(rsemtpm_nikos_control)
number_of_bins <- 19
...
rsemtpm_nikos_chr1_binmedian <- colnames(rsemtpm_nikos_control)
for ( i in 2:length(bins) ) {
  tmp <- rsemtpm_nikos_control[bins[i-1]:bins[i],]
  col_med <- colMedians(tmp, na.rm = T)
  rsemtpm_nikos_chr1_binmedian <- rbind(rsemtpm_nikos_chr1_binmedian, col_med)
}
rsemtpm_nikos_chr1_binmedian <- rsemtpm_nikos_chr1_binmedian[-c(1),]
...
boxplot(df2, use.cols = TRUE, outline=FALSE,
        main="Control Fractional Expression Distribution for Chr1", 
        xlab=sprintf("Gene Bins (%s genes/bin)", as.integer(bin_size)), 
        ylab="Fraction of Median Expression (TPM/TPM)")
...
```{r}
#all samples object
rsemtpm_nikos_chr1 <- rsemtpm_nikos[rsemtpm_nikos$seqnames == 'chr19']
rsemtpm_nikos_geneordered <- rsemtpm_nikos_chr1[order(rsemtpm_nikos_chr1$start),]
...
means <- rowMedians(rsemtpm_nikos_control)
...
for ( i in 1:nrow(rsemtpm_nikos_control )) {
  ...
  for ( j in 1:ncol(rsemtpm_nikos_geneordered) ) {
    rsemtpm_nikos_geneordered[i,j] <- rsemtpm_nikos_geneordered[i,j] - means[i]
  }
}
...
boxplot(df2, use.cols = TRUE, outline=FALSE,
        main="Median Control Subtracted Expression Distribution for Chr1", 
        xlab=sprintf("Gene Bins (%s genes/bin)", as.integer(bin_size)), 
        ylab="Difference from Median Control Expression [TPM]")
...
------------------------------------------------------------------------------
Graph 0.2: adt object
This is the raw gene expression for chr1 with gene quantity based bins.

#NOTES:
1) Etai said that in the z-score visual he takes the median z-score and plots that for bins of 50 genes. First as said above the bins can't be 50 genes. If you are assuming a normal distribution of expression around zero by getting these z-scores. Then you take the median of some normally distributed z-scores you should definitely get something close to the mean? This doesn't seem like a useful image. 


```{r}
#adt_nikos_final$seqnames 
adt_nikos_final <- data.table(adt_nikos_final)
adt_nikos_chr1 <- adt_nikos_final[adt_nikos_final$seqnames == 'chr1']
adt_nikos_chr1_geneordered <- adt_nikos_chr1[order(adt_nikos_chr1$start),]

number_of_genes <- nrow(adt_nikos_chr1_geneordered)
number_of_bins <- 19
...
adt_nikos_chr1_geneordered_noanno <- data.matrix(data.frame(sapply(adt_nikos_chr1_geneordered[,-c(1:4)], function(x) { if(is.factor(x)) {as.numeric(as.character(x))} else {x}})))

adt_nikos_chr1_binsummed <- colnames(adt_nikos_chr1_geneordered[,-c(1:4)])
for ( i in 2:length(bins) ) {
  tmp <- data.matrix(adt_nikos_chr1_geneordered_noanno[bins[i-1]:bins[i],])
  col_med <- colMedians(tmp)
  adt_nikos_chr1_binsummed <- rbind(adt_nikos_chr1_binsummed, col_med)
}
colnames(adt_nikos_chr1_binsummed) <- adt_nikos_chr1_binsummed[c(1),]
adt_nikos_chr1_binsummed <- adt_nikos_chr1_binsummed[-c(1),]
rownames(adt_nikos_chr1_binsummed) <- NULL
adt_nikos_chr1_binsummed <- as.data.frame(adt_nikos_chr1_binsummed)

df <- data.frame(sapply(adt_nikos_chr1_binsummed, function(x) { if(is.numeric(x)==F) {as.numeric(as.character(x))} else {x}}))

df2 <- t(df)

boxplot(df2, use.cols = TRUE, outline=FALSE,
        main="Log2 MeanDiff Distribution for Chr1 (log2(xi) - log2(<xi>))", 
        xlab=sprintf("Gene Bins [%s genes/bin]", as.integer(bin_size)), 
        ylab="Median Expression [log2(TPM)]")
