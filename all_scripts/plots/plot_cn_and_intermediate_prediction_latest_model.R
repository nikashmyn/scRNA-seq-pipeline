...
plot_cn_and_intermediate_prediction <- function(probs, anno_col_numbers = c(1:6), th = 2, main = NA, controlIDs, plot_pvals = T) {
  ...
  #calculating p-vals:
  frac.q <- interstat.arm[sample_id == myid & arm == myqarm]$intermediate_frac
  frac.p <- interstat.arm[sample_id == myid & arm == myparm]$intermediate_frac
  frac.chr <- interstat.chr[sample_id == myid & seqnames== chr]$intermediate_frac
  
  frac.ctrl.q <- interstat.arm[arm == myqarm][sample_id %in% controlIDs]$intermediate_frac
  frac.ctrl.p <- interstat.arm[arm == myparm][sample_id %in% controlIDs]$intermediate_frac
  frac.ctrl.chr <- interstat.chr[seqnames == chr][sample_id %in% controlIDs]$intermediate_frac
  
  pval.p = sum(frac.ctrl.p >= frac.p)/length(frac.ctrl.p)
  pval.q = sum(frac.ctrl.q >= frac.q)/length(frac.ctrl.q)
  pval.chr = sum(frac.ctrl.chr >= frac.chr)/length(frac.ctrl.chr)
  print(pval.chr)

  #pvals.arm <- calc_and_plot_intermediate_pvalue.arm(plotme = F, myid = myid, controlSampleIDs = controlSampleIDs, myseqname = chr)
  #pvals.chr <- calc_and_plot_intermediate_pvalue.chr(plotme = F, myid = myid, controlSampleIDs = controlSampleIDs, myseqname = chr)
  #print((pvals.chr))
  ...
  if(plot_pvals) {
    if(length(myparm)> 0)
      text(x = 0, y = 3.5, labels = sprintf("P-val(%%%d) < %.2f", 
                                            round(frac.p*100), pval.p), adj = 0, cex = 1.35)
    if(length(myqarm)> 0)
      text(x = length(cns), y = 3.5, labels = sprintf("P-val(%%%d) < %.2f", 
                                                      round(frac.q*100), pval.q), adj = 1, cex = 1.35)
    if(length(pval.chr)>0)
      text(x = length(cns)/2, y = 3.5, labels = sprintf("P-val(%%%d) < %.2f", 
                                                        round(frac.chr*100), pval.chr), adj = 1, cex = 1.35)
