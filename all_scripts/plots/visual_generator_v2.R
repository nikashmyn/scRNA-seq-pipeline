...
source(sprintf("%s/scripts/plot_TPM_and_ASE_vis.R", scriptsdir))
...
samples_to_visualize <- c(intersect(columns, anno$WTA.plate))
...
foreach(i = c(1:length(samples_to_visualize))) %dopar% {
  ...
  for(chr in chrs) {
    ...
    plot_TPM_and_ASE(myfamily_file = myfamily_file, myid_file = myid_file, myid = myid, chr = chr, TPM = TPM_bybin, ASE = AS_TPM_bybin, destDir = destDir, controlSampleIDs = controlSampleIDs)
    CumulativeTPM_and_ratioTPM_plots(adt = TPM, geneRanges = geneRanges, controlSampleIDs = controlSampleIDs, myfamily_file = myfamily_file, myid_file = myid_file, myid = myid, chr = chr, destDir = destDir, binsize = 10)
  }
}
...
foreach(i = c(1:length(names(families)))) %dopar% {
  ...
  plot_barplots_of_AllelicAndExpBiasPerSamples(dfs = TPM_bychr2, myfamily_file = fam_id_name, destDir = sprintf("%s/byfamily", destDir),
...
#for(i in 1:2){
  ...
  q <- plot_barplots_of_AllelicAndExpBiasPerSamples(dfs = TPM_bychr2, myfamily_file = fam_id_name, destDir = sprintf("%s/byfamily", destDir),
                                                    wt = rep(1:nrow(TPM_bychr2)), fracs = list(Af = VAR_A, Bf = VAR_B), nogeno = c(),
                                                    ids = myids, name_ids = myids_name, chr = "", plotAxisText = F, return_plot = T)
