...
#anno_gen_1_2$chr_of_interest[is.na(anno_gen_1_2$chr_of_interest)] <- 1 #if no chr interest display chr1
...
for(i in unique(MN_Cell_n_chr_3$family)){
...
visual.data.results.all <- readRDS(file = sprintf("%s/aggregated_results/classification_data.rds", dirpath))
...
ref_AS_TPM_byarm <- readRDS(file = sprintf("%s/aggregated_results/ref_AS_TPM_byarm.rds", dirpath))
...
#ctrl_TPM_melted$vals[which(ctrl_TPM_melted$chr == "10b")] <- ctrl_TPM_melted$vals[which(ctrl_TPM_melted$chr == "10b")]*1.5
#ctrl_x_scale_f <- mean( ctrl_AS_TPM_AB_melted$vals[intersect(which(ctrl_AS_TPM_AB_melted$chr == "23"), which(ctrl_AS_TPM_AB_melted$hap == "A"))] ) + mean( ctrl_AS_TPM_AB_melted$vals[intersect(which(ctrl_AS_TPM_AB_melted$chr == "23"), which(ctrl_AS_TPM_AB_melted$hap == "B"))] )
#ctrl_TPM_melted$vals[which(ctrl_TPM_melted$chr == "23")] <- ctrl_TPM_melted$vals[which(ctrl_TPM_melted$chr == "23")]*(ctrl_x_scale_f/2)
...
setkey(melted_MN_TPM, "variable", "hap", "value")
melted_MN_TPM$chr <- as.character(as.numeric(melted_MN_TPM$chr))
setkey(melted_MN_TPM_chr_spec_5, "variable", "hap", "value")
melted_MN_TPM_w_fam <- merge(melted_MN_TPM, melted_MN_TPM_chr_spec_5[,c("variable", "chr", "hap", "value")], all.x = T)
melted_MN_TPM_w_fam_2 <- melted_MN_TPM_w_fam[-which(melted_MN_TPM_w_fam$chr.x == melted_MN_TPM_w_fam$chr.y),-c("chr.y")]
colnames(melted_MN_TPM_w_fam_2)[colnames(melted_MN_TPM_w_fam_2) == "chr.x"] <- "chr"

#Add a column to melted_MN_TPM_w_fam_2 which shows distance from mean TPM
melted_MN_TPM_grouped <- data.table( melted_MN_TPM_w_fam_2 %>% group_by(chr, hap) %>% mutate(avg_val = mean(value)) %>% mutate(sd_val = sd(value)) %>% mutate(zscore=abs(value-avg_val)/sd_val) ) 
...
#sig_rows <- union(which(melted_MN_TPM_grouped$value>melted_MN_TPM_grouped$upper), which(melted_MN_TPM_grouped$value<melted_MN_TPM_grouped$lower))
sig_rows <- which(melted_MN_TPM_grouped$pval < (.05/24))
...
melted_MN_TPM_chr_spec_5_gen1 <- melted_MN_TPM_chr_spec_5[which(melted_MN_TPM_chr_spec_5$generation == "Gen1"),]
melted_MN_TPM_chr_spec_5_gen1$rupt_time[which(melted_MN_TPM_chr_spec_5_gen1$rupt_time == "early NE defect")] <- "G1"
...
melted_MN_TPM_chr_spec_5_gen2 <- melted_MN_TPM_chr_spec_5[intersect(which(melted_MN_TPM_chr_spec_5$DNA_CN_base %in% c("1","2")), which(melted_MN_TPM_chr_spec_5$generation == "Gen2")),]
melted_MN_TPM_chr_spec_5_gen2$rupt_time[which(melted_MN_TPM_chr_spec_5_gen2$rupt_time == "early NE defect")] <- "G1"
#Five more cases are excluded here for being arm cases. 36 gen2 -> 31 gen2
...
do.call("grid.arrange", c(list(chr_spec_distrs), gen1_summary, gen2_summary, nrow=7))

pdf(file = sprintf("%s/visual_results/summary_visuals/analysis_results.pdf", dirpath), width = 18, height = 12)
do.call("grid.arrange", c(gen1_summary, gen2_summary, ncol=3))
dev.off()
...
ggsave(plot = all_cells_barchart, filename = "MN_event_frequency.pdf", path = sprintf("%s/visual_results/summary_visuals", dirpath), device = "pdf")

##########################
### Test chr12 control ###
##########################

#Might need to change chr12 control distribution but this would change every plot. #Follow up. 
...
#  theme(aspect.ratio = 1/6, text=element_text(size=14), axis.text = element_text(color="black"), axis.title = element_blank(),
#        panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position="none") +
#  geom_errorbar(data = hap_spec_CI_2, mapping = aes(x = chr, ymin = lower, ymax = upper, group = interaction(chr, hap, drop = F)), width=.4, position=position_dodge(.9), alpha=.5)  +
#  geom_errorbar(data = control_CI_2, mapping = aes(x = chr, ymin = lower, ymax = upper), width=.2, alpha=.5)  +
#  geom_jitter(data = melted_MN_TPM_chr_spec_5_gen2, mapping = aes(x = as.character(chr), y = value, group = interaction(chr, hap), color = rupt_time, fill=factor(ifelse((melted_MN_TPM_chr_spec_5_gen2$Interpretation == "no defect"), NA, rupt_time)), shape = DNA_CN_base), position=position_jitterdodge(jitter.width=.05), size = 2.5) + 
#  scale_x_discrete(limits = as.character(intersect(c(1:9, 10.1, 10.2, 11:23), melted_MN_TPM_chr_spec_5_gen2$chr)), labels= as.character(intersect(c(1:9, 10.1, 10.2, 11:23), melted_MN_TPM_chr_spec_5_gen2$chr))) +
#  scale_color_manual(values = c("intact" = "green", "early NE defect" = "yellow", "G1" = "darkorange", "S/G2" = "red")) + 
#  scale_shape_manual(values=c(21,24,22)) +
#  scale_fill_manual(values = c("intact" = "green", "early NE defect" = "yellow", "G1" = "darkorange", "S/G2" = "red"), na.value=NA, guide="none") +
#  labs(x = "Group", y = "TPM Ratio", title = sprintf("Generation 2 Results"))
