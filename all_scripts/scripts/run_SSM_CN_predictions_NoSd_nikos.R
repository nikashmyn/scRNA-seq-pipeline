...
adt <- readRDS("/pellmanlab/stam_niko/data/processed_bam/test/adt.noSd.rds")
adt.na <- readRDS("/pellmanlab/stam_niko/data/processed_bam/test/adt.na.noSd.rds")
...
col_anno <- data.table(read.csv( sprintf("%s/work_in_progress/Annotation_list_long_vnikos_210129.csv", datapath) ))
dim(col_anno)
...
predict_CN <- function(mysample_id, outfname) {
  ...
  source('/pellmanlab/nikos/Stam_Etai_Scripts/scripts/GSSM_utils.R')
  
  run_my_SSM1 <- function() {
    chrsToExcludeFromNormalization = c("chrX", "chrM", "chr10")
    print("Part way with SSM")
    SSM1 <- adt[, lapply(.SD, getMyGSSM),
                .SDcols = mysample_id, by = seqnames]
    print("Half way with SSM")
...
dstdir <- "/pellmanlab/stam_niko/data/processed_bam/test/CN_data"
...
#for(i in 1:10) {
  ...
  if(!is.na(pairid)) {
    File <- sprintf("%s/%s.pair_%s.all_CN_predictions.rds", dstdir, myid, pairid)
  } else {
    File <- sprintf("%s/%s.all_CN_predictions.rds", dstdir, myid)
  }
...
#CN_pred_Sd <- readRDS("/pellmanlab/stam_niko/data/processed_bam/test/CN_data/160627_B1.pair_062716_B1B2.all_CN_predictions.rds")
#CN_pred_noSd <- readRDS("/pellmanlab/stam_niko/data/processed_bam/CN_data/160627_B1.pair_062716_B1B2.all_CN_predictions.rds")
#
#plot(
#  CN_pred_Sd$SSM.TE, CN_pred_noSd$SSM.TE,
#  ylim = c(0,2), xlim = c(0,2)
#)

#Just a Sanity check that the code ran to completion
print("Done with making raw data visuals")
