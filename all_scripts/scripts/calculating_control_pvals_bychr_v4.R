...
# THIS VERSION USES THE SUM IN QUADRATURE OF THE TPM AND VAR SIGNALS. IT ENDED UP GIVING LESS CLEAR DISTRIBUTIONS. 
...
#Change name convention | adt here is actually aggregated TPM and VAR dimension.
adt_bychr <- CN_ratio.bychr
setnames(adt_bychr, "seqnames", "chr")
adt_bychr$chr <- as.character(adt_bychr$chr)
adt_bychr$chr <- paste0("chr", adt_bychr$chr)
adt_bychr <- as.data.frame(adt_bychr, col.names = colnames(adt_bychr))

### get adt values for golden set regions ###

### PICK UP HERE: The next stanza breaks prob because of some formatting thing with the new df above.
