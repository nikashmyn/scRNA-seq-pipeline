...
dir_paths <- list()
for (i in experiments) {dir_paths <- cbind(dir_paths, sprintf("%s/%s/RSEM/output", wkpath, i))}
...
get_tpm_matrix <- function(file, cell_name) {
  ...
  system(sprintf("mkdir -p %s/TPM_data", wkpath, cell_name))
  
  #save data frame with allelic variant coverage
  saveRDS(counts_wcell, sprintf("%s/TPM_data/%s.TPM.rds", wkpath, cell_name))
...
for (i in 1:length(files_samples_v2)) { 
  if (file.exists(sprintf("%s/TPM_data/%s.TPM.rds", wkpath, files_samples_v2[i]))) {
    TPM_cov <- readRDS(sprintf("%s/TPM_data/%s.TPM.rds", wkpath, files_samples_v2[i]))
...
#Save TPM object by gene
saveRDS(TPM, file=sprintf("%s/aggregated_results/TPM.bygene.rds", dirpath))
