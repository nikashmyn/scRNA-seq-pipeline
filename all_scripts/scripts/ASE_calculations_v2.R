...
gtf <- rtracklayer::import(sprintf("%s/refgenomes/Gencode/v25/gencode.v25.primary_assembly.annotation.gtf", datadir))
...
get_allelic_var_matrix <- function(file, cell_name, hets_phase_info) {
  ...
  system(sprintf("mkdir -p %s/ASE_data", wkpath, cell_name))
  
  #save data frame with allelic variant coverage
  saveRDS(Allelic_var_cov, sprintf("%s/ASE_data/%s.ASE.rds", wkpath, cell_name))
...
for (i in 1:length(files_samples_v2)) { 
  if (file.exists(sprintf("%s/ASE_data/%s.ASE.rds", wkpath, files_samples_v2[i]))) {
    Allelic_var_cov <- readRDS(sprintf("%s/ASE_data/%s.ASE.rds", wkpath, files_samples_v2[i]))
...
saveRDS(ASE_all_cells, file=sprintf("%s/aggregated_results/ASE_all_cells.rds", dirpath))
ASE_all_cells <- readRDS(file=sprintf("%s/aggregated_results/ASE_all_cells.rds", dirpath))
