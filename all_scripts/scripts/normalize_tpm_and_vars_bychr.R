...
adt <- fromRawTPMtoExprsRatio(rsemtpm, geneRanges, controlSampleIDs, max_pos_col = 6, plusOne = 1, 
                              ...
                              maxExp = 1200, quantileOfExp = 0.99, minDetectionLevel = configs$minDetectionLevel, minNumOfSamplesToDetect = 5 )$adt

#Var cnts per 50 genomically sequential genes for allele A and B
varA_mat <- var_mat_A <- coding$cnts.A
varB_mat <- var_mat_B <- coding$cnts.B
...
varA_mat <- varA_mat[order(seqnames, bin)]
...
varB_mat <- varB_mat[order(seqnames, bin)]
