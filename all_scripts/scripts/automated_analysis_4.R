...
ref_AS_TPM <- readRDS(file = sprintf("%s/aggregated_results/ref_AS_TPM_unmelt.rds", dirpath))
...
#save event table
write.csv(all_family_events_3, file = sprintf("%s/aggregated_results/all_family_events.csv", dirpath), row.names = F)

#arm values for manual entry
AS_TPM_byarm <- readRDS(file=sprintf("%s/aggregated_results/AS-TPM.inv_var.byarm.rds", dirpath))
AS_TPM_byarm_ctrl_pvals_arm_spec <- readRDS(file = sprintf("%s/aggregated_results/AS_TPM_pval_matrix_control_byarm_armspec.rds", dirpath))
